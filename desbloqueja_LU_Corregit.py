...
def LUpivesg(A, tol=1.e-10):
    ...
    for k in range(0, n - 1):
        ...
        #seleccionem el pivot
        # Oscar me ha puesto un comentario diciendo que el np.max selecciona el maximo de las filas en A[k:,:]
        indexPivot = k + np.argmax(abs(A[k:, k]) / np.max(abs(A[k:, k:]), axis=1))
